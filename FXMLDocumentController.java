...
public class FXMLDocumentController extends Application {
  ...
  @Override public void start(Stage stage) {
    ...
    stage.setTitle("javafxcalculator");
  ...
  private TextField createScreen() {
    ...
    screen.setStyle("-fx-background-color: orange;");
  ...
  private TilePane createButtons() {
    // b is button is setting it title
    TilePane b = new TilePane();
    b.setVgap(7);
    b.setHgap(7);
    b.setPrefColumns(template[0].length);
    for (String[] row: template) {
      for (String s: row) {
        // button .children is going into that and creating and is creating button
        b.getChildren().add(createButton(s));
      }
    }
    return b;
  }
